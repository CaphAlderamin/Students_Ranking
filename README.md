...
- **Качество**: PHPUnit (юнит + интеграционные round-trip), PHPStan level max + strict-rules, покрытие pcov ([отчёт docs/coverage.md](docs/coverage.md)), CI, автогенерация mermaid-схемы из DBML.
...
Источник истины — [database/schema.dbml](database/schema.dbml). Mermaid-диаграмма автогенерируется (`make dbml`, см. [docs/schema.mmd](docs/schema.mmd)); блок ниже — зеркало `docs/schema.mmd` (автоподдержка между маркерами B5-05):
...
Подробный отчёт о покрытии ядра — [docs/coverage.md](docs/coverage.md) (pcov, дерево покрытия по классам).
